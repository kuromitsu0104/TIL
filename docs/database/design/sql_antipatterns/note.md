...
- [Part16: プアマンズ・サーチエンジン(貧者のサーチエンジン)](#part16-プアマンズサーチエンジン貧者のサーチエンジン)
  ...
  - [まとめ](#まとめ-3)
- [Part:19 リーダブルパスワード(読み取り可能パスワード)](#part19-リーダブルパスワード読み取り可能パスワード)
  - [目的: パスワードのリカバリーとリセットを行う](#目的-パスワードのリカバリーとリセットを行う)
  - [アンチパターン: パスワードを平文で格納する](#アンチパターン-パスワードを平文で格納する)
  - [パスワードの認証](#パスワードの認証)
  - [パスワードを電子メールで送信する](#パスワードを電子メールで送信する)
  - [アンチパターンの見つけ方](#アンチパターンの見つけ方-17)
  - [アンチパターンを用いてもよい場合](#アンチパターンを用いてもよい場合-16)
  - [解決策: ソルトをつけてパスワードハッシュを格納する](#解決策-ソルトをつけてパスワードハッシュを格納する)
    - [ハッシュ関数を理解する](#ハッシュ関数を理解する)
    - [SQL でのハッシュの使用](#sql-でのハッシュの使用)
    - [ハッシュにソルトを加える](#ハッシュにソルトを加える)
    - [SQL からパスワードを隠す](#sql-からパスワードを隠す)
    - [パスワードをリカバリーするのではなく、リセットする](#パスワードをリカバリーするのではなくリセットする)
- [Part20: SQL インジェクション](#part20-sql-インジェクション)
  - [目的: 動的 SQL を記述する](#目的-動的-sql-を記述する)
  - [アンチパターン: 未検証の入力をコードとして実行する](#アンチパターン-未検証の入力をコードとして実行する)
    - [アクシデントは起きる](#アクシデントは起きる)
    - [ウェブ最大のセキュリティ脅威](#ウェブ最大のセキュリティ脅威)
    - [対処法の追求](#対処法の追求)
      - [値のエスケープ](#値のエスケープ)
      - [プリペアドステートメント](#プリペアドステートメント)
      - [ストアドプロシージャ](#ストアドプロシージャ)
      - [データベースフレームワーク](#データベースフレームワーク)
  - [アンチパターンの見つけ方](#アンチパターンの見つけ方-18)
  - [アンチパターンを用いてもよい場合](#アンチパターンを用いてもよい場合-17)
  - [解決策: 誰も信用してはならない](#解決策-誰も信用してはならない)
    - [入力のフィルタリング](#入力のフィルタリング)
    - [動的値のパラメータ化](#動的値のパラメータ化)
    - [動的値を引用符で囲む](#動的値を引用符で囲む)
    - [ユーザーの入力をコードから隔離する](#ユーザーの入力をコードから隔離する)
    - [他の開発者にコードをレビューしてもらう](#他の開発者にコードをレビューしてもらう)
  - [まとめ](#まとめ-4)
- [Part21: シュードキー・ニートフリーク(擬似キー潔癖症)](#part21-シュードキーニートフリーク擬似キー潔癖症)
  - [目的: 欠番を詰める](#目的-欠番を詰める)
  - [アンチパターン; 隙間を埋める](#アンチパターン-隙間を埋める)
    - [欠番を割り当てる](#欠番を割り当てる)
    - [既存行の番号を振り直す](#既存行の番号を振り直す)
    - [データ不一致の元](#データ不一致の元)
  - [アンチパターンの見つけ方](#アンチパターンの見つけ方-19)
  - [アンチパターンを用いてもよい場合](#アンチパターンを用いてもよい場合-18)
  - [解決策: 擬似キーの欠番は埋めない](#解決策-擬似キーの欠番は埋めない)
    - [行のナンバリング](#行のナンバリング)
    - [GUID の使用](#guid-の使用)
    - [最も重要な問題](#最も重要な問題)
  - [まとめ](#まとめ-5)
- [Part22: シー・ノー・エビル(臭いものに蓋)](#part22-シーノーエビル臭いものに蓋)
  - [目的: 簡潔なコードを書く](#目的-簡潔なコードを書く)
  - [アンチパターン: 肝心な部分を見逃す](#アンチパターン-肝心な部分を見逃す)
    - [診断せずに判断する](#診断せずに判断する)
    - [見逃しがちなコード](#見逃しがちなコード)
  - [アンチパターンの見つけ方](#アンチパターンの見つけ方-20)
  - [アンチパターンを用いてもよい場合](#アンチパターンを用いてもよい場合-19)
  - [解決策: エラーから優雅に回復する](#解決策-エラーから優雅に回復する)
    - [リズムを維持する](#リズムを維持する)
    - [ステップをたどり直す](#ステップをたどり直す)
- [Part23: ディプロマティック・イミュニティ(外交特権)](#part23-ディプロマティックイミュニティ外交特権)
  - [目的: ベストプラクティスを採用する](#目的-ベストプラクティスを採用する)
  - [アンチパターン: SQL を特別扱いする](#アンチパターン-sql-を特別扱いする)
  - [アンチパターンの見つけ方](#アンチパターンの見つけ方-21)
  - [アンチパターンを用いてもよい場合](#アンチパターンを用いてもよい場合-20)
  - [解決策:包括的に品質管理に取り組む](#解決策包括的に品質管理に取り組む)
    - [文書化](#文書化)
    - [バージョン管理](#バージョン管理)
    - [テスティング](#テスティング)
- [Part24: マジックビーンズ(魔法の豆)](#part24-マジックビーンズ魔法の豆)
  - [目的: MVC の M(モデル)を単純化する](#目的-mvc-の-mモデルを単純化する)
  - [アンチパターン: モデルがアクティブレコードそのもの](#アンチパターン-モデルがアクティブレコードそのもの)
    - [アクティブレコードはモデルをデータベーススキーマに強く依存させてしまう](#アクティブレコードはモデルをデータベーススキーマに強く依存させてしまう)
    - [アクティブレコードは CRUD 機能を公開してしまう](#アクティブレコードは-crud-機能を公開してしまう)
    - [アクティブレコードはドメインモデル貧血症をもたらす](#アクティブレコードはドメインモデル貧血症をもたらす)
    - [マジックビーンズのユニットテストは困難](#マジックビーンズのユニットテストは困難)
  - [アンチパターンの見つけ方](#アンチパターンの見つけ方-22)
  - [アンチパターンを用いてもよい場合](#アンチパターンを用いてもよい場合-21)
  - [解決策: モデルがアクティブレコードを「持つ」ようにする](#解決策-モデルがアクティブレコードを持つようにする)
    - [モデルを理解する](#モデルを理解する)
    - [ドメインモデルの使用](#ドメインモデルの使用)
    - [プレーンなオブジェクトのテスト](#プレーンなオブジェクトのテスト)
    - [現実的に考える](#現実的に考える)
  - [まとめ](#まとめ-6)
- [Part25: 砂の城](#part25-砂の城)
  - [アンチパターン: 想定不足](#アンチパターン-想定不足)
  - [アンチパターンの見つけ方](#アンチパターンの見つけ方-23)
  - [アンチパターンを用いてもよい場合](#アンチパターンを用いてもよい場合-22)
  - [解決策](#解決策)
    - [ベンチマーク](#ベンチマーク)
    - [テスト環境の構築](#テスト環境の構築)
    - [例外処理](#例外処理)
...
# Part:19 リーダブルパスワード(読み取り可能パスワード)
...
- MySQL では`SHA2関数`を使用することで、入力値を`SHA-256`でハッシュ化した値を格納できる
- よく利用するパターン
  - パスワードをハッシュ化して DB に格納
  - ユーザーの入力値をハッシュ化して DB 内のハッシュ化されたパスワードと比較して妥当性を確認

### ハッシュにソルトを加える

- `ソルト`という任意の文字列をパスワードに連結してからハッシュ化する手法のこと
- 実施する理由
  - `SHA-256`だけでは辞書攻撃への対策として不十分
  - 時間をかければ突破できてしまう
- メリット
  - 辞書攻撃の対象となるようなパスワードにならないため
- 実施方法
  - テーブルに`salt列`を追加して、パスワードに連結したソルトを格納
  - ハッシュ化したパスワードである`password_hash列`には、パスワードとソルトを連結した値を SHA-256 でハッシュ化した値を格納
  - パスワードの妥当性の検証では、入力値とソルトを連結した値をハッシュ化して行う

### SQL からパスワードを隠す

- アプリケーション側で DB からソルトを取得して、パスワードをハッシュ化してから SQL を実行する
...
**値のリストを一つのパラメータにすることはできない**

- カンマ区切りで値を渡しても、文字列のパラメータとして認識されてしまう
  - 理想
    - `SELECT * FROM Projects WHERE project_name IN (123, 345, 567)`
  - 実態
    - `SELECT * FROM Projects WHERE project_name IN ('123,345,567')`
- MySQL では最初の文字列だけ処理される

**テーブル識別子もパラメータとして扱えない**

- テーブル名としてではなく、文字リテラルと判断されて構文エラーとなる
  - 理想
    - `SELECT * FROM Projects WHERE project_id = 1234`
  - 実態
    - `SELECT * FROM 'Projects' WHERE project_id = 1234`

**列名もパラメータとして扱えない**

- `ORDER BY`などで列名を指定してソートしたいが、列名が文字リテラルとして認識されてソートされない
  - 理想
...
- 動的に SQL ステートメントを構築する場合、SQL インジェクションの危険を常に考慮すること
...
- ユーザーからの入力で不要なものは必ず取り除くこと
- 必要な部分だけ利用すること
- 数値型の場合は、型キャストで対策
- 文字列型の場合は、正規表現で値を抽出して対策

### 動的値のパラメータ化

- 動的な部分がシンプルな場合は、プリペアドステートメントを利用可能
- Rails などのメジャーなライブラリでは標準で導入済み

### 動的値を引用符で囲む

### ユーザーの入力をコードから隔離する

- ユーザーの入力値を使って、アプリケーション側で定義済みの値を参照して SQL ステートメントを構築する方法
...
- コードレビューでの SQL インジェクションについての観点
  1. 動的に SQL ステートメントを構築している箇所を特定
  2. 入力ソースの特定
  3. 入力ソースのサニタイズ有無、手法をチェック

## まとめ

- 「ユーザーには、値の入力は許可しても、コードの入力を許可してはいけません。」

# Part21: シュードキー・ニートフリーク(擬似キー潔癖症)
...
- デメリット
  - 欠番を特定するクエリを実行しなければならないこと
  - 並列処理だと欠番の特定処理で不整合が発生してすること

### 既存行の番号を振り直す

- デメリット
  - 競合状態を引き起こす可能性があること
  - テーブル定義の制約により、何度も処理を実行しなければならないこともある
...
- よくある発言
  - 「挿入をロールバックした後で、自動生成された ID 値を再利用するにはどうすればいい？」
    - 擬似キーの割り当てのロールバックはできない
  - 「bug_id の 4 番で何が起こったの？」
    - 欠番に対して根拠のない不安を感じている
  - 「使用されていない最初の ID を取得するクエリはどうやって書いたらいい？」
    - ID の再割り当てをしようとしようとしている
  - 「番号が足らなくなったらどうしよう？」
    - ID 値の再割り当てを正当化しようとしている
...
- 「擬似キーは、行を一意に識別するためにあります。行番号と近藤しないようにしましょう。」

# Part22: シー・ノー・エビル(臭いものに蓋)
...
- SQL ステートメントをアプリケーション側で構築する場合は、一時変数を使って SQL ステートメントを構築してから行うこと

# Part23: ディプロマティック・イミュニティ(外交特権)

## 目的: ベストプラクティスを採用する

- ソースコードのバージョン管理
- ユニットテストや機能テストの自動化
- ドキュメント、仕様書、コードコメントの記述、アプリケーションの要件や実装戦略を記録
...
- 品質保証(Quality Assurance: QA)には 3 つの要素がある
...
- ActiveRecord とは
  - テーブルに対する CRUD 操作とモデルをマッピングするデザインパターンのこと

### アクティブレコードはモデルをデータベーススキーマに強く依存させてしまう

- データベースを変更したときに、アプリケーション側のコードも修正しなければならない

### アクティブレコードは CRUD 機能を公開してしまう

- CRUD 操作に関連するルールを定義していても、CRUD 操作を直接行えるためルールを回避できてしまう
...
- 「複雑なモデル操作を全てのコントローラにコピーするにはどうすれば良いのか？」
  - モデル内で複雑な操作をコーディングして、インターフェースの一部として公開するべき
...
- MVC の M の本来の意味合いは、対象ドメインに対するビジネスルールやビジネスロジックを実装する場所であること
- 複数のモデルに操作がまたがる場合は、関連するモデルの情報を全て持つモデルを定義してビジネスロジックを実装する

  ```mermaid
  flowchart TB

  Adminコントローラ-->BugReport
  DataEntryコントローラ-->BugReport
  Displayコントローラ-->BugReport
  Searchコントローラ-->BugReport
  BugReport-->Bugsアクティブレコード
  BugReport-->Accountsアクティブレコード
  BugReport-->Productsアクティブレコード
  ```
